  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center px-4">
      <div className="max-w-md text-center">
        ...
        <Link to="/" className="mt-6 inline-block rounded-full bg-[var(--navy)] px-6 py-2.5 text-sm text-[var(--cream)] hover:opacity-90">Kembali ke Beranda</Link>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center px-4 text-center">
      <div>
        ...
        <button onClick={() => { router.invalidate(); reset(); }} className="mt-4 rounded-full bg-[var(--navy)] px-6 py-2 text-sm text-[var(--cream)]">Coba lagi</button>
      </div>
    </div>
  );
}
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Cafe minimalis premium. Nikmati kopi hangat berkualitas, suasana tenang, dan layanan via GoFood, GrabFood, dan ShopeeFood." },
